...
public class EvaluationService {
	...
	public String acronym(String phrase) {
		if (phrase.isEmpty()) {
			...
			StringBuilder str = new StringBuilder();
			for (String s : phrase.split("[^\\w]+")) {
				str.append(s.charAt(0));
			}
			return str.toString().toUpperCase().replaceAll("[^a-zA-Z]", "");
		}
	}
	...
	public int getScrabbleScore(String string) {
		...
		for (char l : letters) {
			if (onePt.indexOf(l) != -1) {
				points += 1;
			} else if (twoPt.indexOf(l) != -1) {
				points += 2;
			} else if (threePt.indexOf(l) != -1) {
				points += 3;
			} else if (fourPt.indexOf(l) != -1) {
				points += 4;
			} else if (fivePt.indexOf(l) != -1) {
				points += 5;
			} else if (eightPt.indexOf(l) != -1) {
				points += 8;
			} else {
				points += 10;
			}
	...
	public Map<String, Integer> wordCount(String string) {
		...
		for (String s : string.replace("\n", "").split("[\\W_]")) {
			if (words.containsKey(s)) {
				int curVal = words.get(s);
				words.replace(s, ++curVal);
			} else {
				words.put(s, 1);
			}
		}
	...
	static class BinarySearch<T> {
		...
		public int indexOf(T t) {
			int result = Arrays.binarySearch(this.sortedList.toArray(), t);
			return result;
		}
	...
	public String toPigLatin(String string) {
		...
		for (String word : words) {
			...
			vowelLoop: for (int i = 0; i < word.length(); i++) {
			...
			if (index != 0) {
				String first = word.substring(0, index), end = word.substring(index, word.length());
				str.append(end + first + "ay");
	...
	public boolean isArmstrongNumber(int input) {
		...
		default:
			...
			for (String num : digits) {
				result += Math.pow(Integer.parseInt(num), numDigits);
			}
			if (result == input) {
				return true;
			} else {
		...
		public String rotate(String string) {
			...
			for (int i = 0; i < string.length(); i++) {
				if (Character.isDigit(string.charAt(i))
						|| !Character.isAlphabetic(string.charAt(i)) && !Character.isDigit(string.charAt(i))) {
	...
	public int calculateNthPrime(int n) {
		...
		if (n <= 0) {
			...
			while (count < n) {
				...
				for (i = 2; i <= num; i++) {
					// attempt to find next prime
					if (num % i == 0) {
						break;
					}
				}

				if (i == num) {// it is a prime number
					++count;
				}
			}
	...
	static class AtbashCipher {
		...
		public static String encode(String string) {
			...
			for (int i = 0; i < string.length(); i++) {
				if (Character.isAlphabetic(string.charAt(i))) {
					...
					// replace with character from atbash
					str.append(atbash.charAt(index));
				} else {
					str.append(string.charAt(i));
				}
				// output in groups of 5 (no space after last character)
		...
		public static String decode(String string) {
			...
			for (int i = 0; i < string.length(); i++) {
				if (Character.isAlphabetic(string.charAt(i))) {
					// get index of current character
					int index = atbash.indexOf(string.charAt(i));
					// replace with character from atbash
	...
	public boolean isValidIsbn(String string) {
		char[] isbn = string.replaceAll("[^A-Za-z0-9]", "").toCharArray();
		int check = 0;
		if (isbn.length > 10) {
			return false;
		} else {
			for (int i = 0; i < isbn.length; i++) {
				...
				check += (Character.getNumericValue(x) * (isbn.length - i));
			}
			if (check % 11 == 0) {
				return true;
			} else {
	...
	public boolean isPangram(String string) {
		...
		for (char letter : string.toLowerCase().replace(" ", "").toCharArray()) {
			found.add(letter);
		}
		if (found.size() == 26) {
			return true;
